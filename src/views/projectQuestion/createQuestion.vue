<template>
  <div class="createQuestion" v-loading="Data.isLoading">
    <div class="Topping" :style="{ height: `calc(100vh - 180px)`, overflowY: 'scroll' }">
      <div class="form-box">
        <FormLayout ref="form" labelWidth="auto" :formList="Data.formList" :submitData="Data.submitData">
          ...
          <template #responsibleUserReference="{ option }">
            <SelectedDialog
              title="选择负责人"
              :allText="$intl('userList').d('用户列表')"
              v-model:isShow="option.showDialog"
    ...
    <el-affix target=".createQuestion" position="bottom" :offset="0">
      <div class="actionRow">
...
<script setup name="createQuestion">
...
const changeSubmitData = (select, option) => {
  ...
  if (option.name === 'addedBy') {
    ...
    Data.actualIssueHandler = sel.pdcUser;
    Data.submitData[option.name] = sel.wcUser;
  } else {
...
const getCreateLayout = async () => {
  let params = {
    ...
    softType: 'com.pisx.pmgt.change.PIProjectIssue',
  };
  ...
  if (layoutData.success) {
    ...
    datas.forEach((v) => {
      v.items.forEach((e) => {
        ...
        // else if (e.name === 'planActivityReference') {
        //   e.filedType = 'INPUT';
        // }
      });
    });
