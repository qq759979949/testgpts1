    ...
    <levelTable :columns="Data.afterColums" :tableData="Data.tableData" :tableLoad="Data.tableLoad" :border="true" :isCheckBox="false" :columnIndex="0" :isAddColumn="true">
      ...
      <template #actuallyValue="scope">
        <span v-if="Data.state === '工作进行中' && route.name === 'target'">
          <el-input v-model="scope.row.actuallyValue" placeholder="请输入" controls-position="right" size="small" v-if="(Data.groupFlag && Data.roleFlag) || Data.allRoleName" />
      ...
      <template #targetIsCompliance="scope">
        <span v-if="Data.state === '工作进行中' && route.name === 'target'">
          <el-select v-model="scope.row.targetIsCompliance" style="width: 100%" v-if="(Data.groupFlag && Data.roleFlag) || Data.allRoleName">
      ...
      <template #targetFileName="scope">
        <span v-if="Data.state === '工作进行中' && route.name === 'target'">
          <span v-if="(Data.groupFlag && Data.roleFlag) || Data.allRoleName">
      ...
      <template #isCompliance="scope">
        <span :class="scope.row.isCompliance === '否' ? 'compliance' : ''">{{ scope.row.isCompliance }}</span>
      </template>
    </levelTable>
  </div>
  <!-- 上传文件 -->
...
const handlePresent = () => {
  ...
  newObj.append('targetIndicatorsInfos', JSON.stringify(Data.tableData));
  newObj.append('oid', Data.oid);
  newObj.append('indicatorType', '经济效益指标');
  api.targetApi.saveTargetIndicators(newObj).then((res) => {
    if (res.success) {
