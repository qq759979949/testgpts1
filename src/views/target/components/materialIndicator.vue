  ...
  <div>
    ...
    >
      ...
      <template #acceptancePhase="scope">
        <el-select v-model="scope.row.acceptancePhase" placeholder="请选择" style="width: 100%" v-if="scope.row.editFlag || scope.row.addFlag === 'true'" multiple>
          <el-option v-for="item in props.acceptancePhaseList" :key="item.name" :label="item.name" :value="item.name" />
        </el-select>
        <span v-else>{{ scope.row.acceptancePhase }}</span>
      </template>
      <template #weight="scope">
        <el-input v-model="scope.row.weight" placeholder="请输入" v-if="scope.row.editFlag || scope.row.queryFlag" />
        <span v-else>{{ scope.row.weight }}</span>
      </template>
      <template #exCompetitorsTargetVal="scope">
        <el-input v-model="scope.row.exCompetitorsTargetVal" placeholder="请输入" v-if="scope.row.editFlag || scope.row.queryFlag" />
        <span v-else>{{ scope.row.exCompetitorsTargetVal }}</span>
      </template>
      <template #inCompetitorsTargetVal="scope">
        <el-input v-model="scope.row.inCompetitorsTargetVal" placeholder="请输入" v-if="scope.row.editFlag || scope.row.queryFlag" />
...
<script setup>
import { materialIndicatorColums } from '../tableColunms';
...
const props = defineProps({
  ...
  technicalColumns: {
    type: Array,
    default: () => materialIndicatorColums,
  },
  // 产品类别
  productCategoryList: {
    type: Array,
  ...
  api.targetApi.searchTargetTemplates(params).then((res) => {
    if (res.success) {
      ...
      Data.templateTableData = newArr.map((item) => {
        item.exCompetitorsTargetVal = '';
        item.inCompetitorsTargetVal = '';
...
const handlePresent = () => {
  ...
  array.map((obj) => {
    obj.editFlag = false;
    if ((Array.isArray(obj.acceptancePhase) && obj.acceptancePhase.length > 0) || (Array.isArray(obj.actualValueProvided) && obj.actualValueProvided.length > 0)) {
      obj.acceptancePhase = obj.acceptancePhase.join(',');
