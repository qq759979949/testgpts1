<template>
  <div class="plan_deliver">
    <el-drawer v-model="Data.isShowDrawer" title="交付物操作" @close="closeFun" :size="1000">
      <section v-if="isBaseLineDetail">
            ...
            <template #default="{ row, $index }">
              <!-- <div v-if="row.oid">
                ...
                <span v-if="row.isEdit" style="margin-left: 20px">
                  <el-icon color="#386bd7" style="cursor: pointer" @click="uploadClick($index)"><Upload /></el-icon>
                </span>
              </div>
              <div v-else class="deliver">
                <span v-if="row.subjectName">
                  {{ row.subjectName }}
                  <span>
                    <el-icon @click="uploadClick($index)" color="#386bd7" style="margin-left: 20px; cursor: pointer"><Upload /></el-icon>
                  </span>
                </span>
              ...
              <div class="view_box">
                ...
                <el-tooltip content="预览" placement="top" v-if="row.subjectName">
                  <el-icon v-prview="{ id: row.dataOid, name: row.fileName }" class="iconClose"><View /></el-icon>
                </el-tooltip>
              </div>
            </template>
          </el-table-column>
...
const isDefaultPosition = ref('');
const uploadClick = (idx) => {
  parmasObj.value = {};
  isIndex.value = idx;

  api.planApi.getParentActivity({ oid: props.oid }).then((res) => {
    if (res.success) {
      isDefaultPosition.value = res.data.activityName;
...
const edit = () => {
  if (selectData.value.length === 0) return window.$message.warning('请选择数据编辑！');
  let isFromTemplateData = selectData.value.some((e) => e.isFromTemplate === '是');
  if (isFromTemplateData) window.$message.warning('模板带出来的数据不能编辑！');
  Data.list.forEach((e) =>
    selectData.value.forEach((v) => {
      if (e.oid === v.oid && v.isFromTemplate === '否') {
        e.isEdit = true;
      }
    }),
  );
...
// 移除
const remove = () => {
  let isCanDelete = selectData.value.some((v) => v.isCut === '否');
  if (isCanDelete) return window.$message.warning('请选择能够裁剪的数据！');
...
const closeFun = () => {
  ...
  emit('update:closeFun', false);
};
/**保存 */
const save = () => {
  console.log(Data.list, 253);
  ...
  newArr.forEach((e, i) => {
    ...
    if (!e?.type || e?.type === 'linkFile') {
      linkFile = {
        ...commonParmas,
        objectOid: e?.accessDialogObj?.oidOrVid ? e?.accessDialogObj?.oidOrVid : e.subjectOid,
        operateUser: Data.userInfo.username,
      };
 ...
 * emit输出
 */
const emit = defineEmits(['update:closeFun', 'confirm', 'updateDeliver']);
