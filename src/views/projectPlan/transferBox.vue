    ...
    <el-dialog v-model="isShowDialog" :title="title" width="800" @close="close">
                    ...
                    <template #item="{ element, index }">
                      <el-checkbox :label="element[dataKey]">
                        <div class="check-row">
                          ...
                          <div class="row-text row-mover" v-show-tip>
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button type="primary" link class="fl" @click="resetDefault">
...
export default {
  ...
  methods: {
    ...
    toRight() {
      ...
      this.leftCheckedList = [];
      this.indeterminate_left = false;
      this.isCheckedAll_left = false;
    },
    // 左移按钮
    toLeft() {
      if (this.rightCheckedList.length === this.rightList.length) return window.$message.warning('最少选择一个！');
    ...
    resetDefault() {
      this.leftCheckedList = [];
      this.leftCheckedList = this.allList.filter((el) => this.defaultList.every((v) => v.key !== el.key));
