  ...
  <div>
    ...
    <div v-else>
      ...
      >
        ...
        <!-- <template #indicatorName="scope" v-else>
          <div v-if="scope.row.isDelete" class="text-line">{{ scope.row.indicatorName }}</div>
          <div v-else>{{ scope.row.indicatorName }}</div>
        </template> -->
        ...
        <template #formula="scope">
          <el-input v-model="scope.row.formula" placeholder="请输入" v-if="scope.row.editFlag" />
...
<script setup>
import { qualityColums, otherQualityColums } from '../tableColunms';
...
onMounted(() => {
  ...
  if (props.isChange) {
    Data.colunms = props.qualityColumns;
  } else {
    Data.colunms = Data.projectType === '产品开发类项目' ? qualityColums : otherQualityColums;
  }
  if (!props.isChange) {
    getTargetType();
  }
